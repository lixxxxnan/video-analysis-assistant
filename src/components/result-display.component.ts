...
@Component({
  ...
  imports: [],
  template: `
    <div class="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 relative print:shadow-none print:border-none print:rounded-none">
      
      <!-- Toast Notification -->
      @if (showToast()) {
        <div class="fixed top-24 left-1/2 transform -translate-x-1/2 bg-gray-900/90 backdrop-blur-sm text-white px-6 py-3 rounded-full shadow-2xl z-50 flex items-center gap-2 animate-[pulse_0.2s_ease-out] print:hidden">
      ...
      <!-- Title Header -->
      <div class="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 text-white flex justify-between items-start print:bg-none print:text-black print:p-0 print:mb-6">
        <div>
          <h2 class="text-2xl font-bold leading-tight mb-2">{{ data().title }}</h2>
          <div class="flex items-center gap-2 opacity-90 text-sm print:text-gray-600">
            <span class="px-2 py-0.5 bg-white/20 rounded text-xs font-medium print:bg-gray-100 print:text-gray-800">AI 分析结果</span>
            <span class="print:hidden text-xs opacity-80">Generated by Gemini</span>
          </div>
        </div>
      </div>

      <div class="p-6 space-y-8 print:p-0 print:space-y-6">
        
        <!-- AI Summary -->
        <section class="break-inside-avoid">
          <h3 class="text-sm font-bold text-gray-400 uppercase tracking-wider mb-3 flex items-center gap-2 print:text-black">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25H12" />
            </svg>
            视频内容概览
          </h3>
          <div class="bg-white p-5 rounded-xl border border-gray-200 text-gray-800 leading-relaxed text-base shadow-sm print:shadow-none print:border-gray-300 print:p-4">
            {{ data().summary }}
          </div>
        </section>
        ...
        @if (data().keyHighlights.length > 0) {
          <section class="break-inside-avoid">
             <h3 class="text-sm font-bold text-gray-400 uppercase tracking-wider mb-3 flex items-center gap-2 print:text-black">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-blue-600 print:text-black">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 18v-5.25m0 0a6.01 6.01 0 001.5-.189m-1.5.189a6.01 6.01 0 01-1.5-.189m3.75 7.478a12.06 12.06 0 01-4.5 0m3.75 2.383a14.406 14.406 0 01-3 0M14.25 18v-.192c0-.983.658-1.823 1.508-2.316a7.5 7.5 0 10-7.517 0c.85.493 1.509 1.333 1.509 2.316V18" />
              </svg>
              关键亮点
            </h3>
            <div class="bg-blue-50 rounded-xl p-5 border border-blue-100 print:bg-white print:border-gray-300 print:p-4">
              <ul class="space-y-3">
                @for (point of data().keyHighlights; track $index) {
                  <li class="flex items-start gap-3">
                    <span class="inline-flex items-center justify-center h-5 w-5 rounded-full bg-blue-600 text-white text-xs font-bold shrink-0 mt-0.5 print:bg-black print:text-white">{{ $index + 1 }}</span>
        ...
        <section>
          <div class="flex items-center justify-between mb-3">
            <h3 class="text-sm font-bold text-gray-400 uppercase tracking-wider flex items-center gap-2 print:text-black">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
              </svg>
              图文笔记
            </h3>
             <button class="text-xs font-medium text-blue-600 hover:text-blue-700 transition-colors flex items-center gap-1 group bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100 hover:bg-blue-100 print:hidden" (click)="copyText(data().transcript)">
          ...
          <div class="space-y-6 print:space-y-4">
            @for (segment of data().noteSegments; track $index) {
              <div class="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow break-inside-avoid print:shadow-none print:border print:border-gray-300 print:rounded-lg">
                <div class="flex flex-col md:flex-row print:flex-row">
                  <!-- Screenshot Area -->
                  <div class="md:w-1/3 h-48 md:h-auto bg-gray-100 relative shrink-0 print:w-1/3 print:h-40">
                    @if (screenshots().get($index)) {
                      ...
                      <div class="absolute bottom-2 right-2 bg-black/60 text-white text-xs px-2 py-1 rounded backdrop-blur-sm print:hidden">
                        {{ segment.timeLabel }}
                      </div>
                    } @else {
                  ...
                  <!-- Content Area -->
                  <div class="p-5 md:w-2/3 flex flex-col justify-center print:w-2/3 print:p-4">
                    <h4 class="font-bold text-gray-900 mb-2">{{ segment.title }}</h4>
                    <p class="text-gray-600 text-sm leading-relaxed whitespace-pre-wrap print:text-gray-900">{{ segment.content }}</p>
                  </div>
                </div>
              </div>
            }
